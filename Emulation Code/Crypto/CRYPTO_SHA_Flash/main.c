...
int32_t RunSHA()
{
    ...
    /* Copy the data in SRAM to Flash */
    //printf("len = %d\n", g_i32DataLen/8);
    FMC->ISPCTL = 0x7d;
    FMC_Erase(0x70000);
    for(i = 0; i < (g_i32DataLen + 31) / 32; i++)
    {
        FMC_Write(0x70000 + i * 4,  M32((uint32_t)&g_au8ShaData[i * 4]));
    }


    SHA_Open(CRPT, SHA_MODE_SHA1, SHA_IN_SWAP, 0);
    g_i32DigestLength = 160/8;

    //SHA_SetDMATransfer(CRPT, (uint32_t)&g_au8ShaData[0], g_i32DataLen / 8);
    SHA_SetDMATransfer(CRPT, 0x70000, g_i32DataLen / 8);
