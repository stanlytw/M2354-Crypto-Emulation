...
__ALIGNED(32)   uint8_t  g_hmac_msg[1024];
__ALIGNED(32)   uint8_t  g_hmac_mac[1024];
...
int  get_next_pattern(void)
{
    ...
    while(get_line() == 0)
    {
        ...
        if(strncmp(g_line, "[L=", 3) == 0)
        {
            ...
            {
                case 20:
                    g_sha_mode = (SHA_MODE_SHA1 << CRPT_HMAC_CTL_OPMODE_Pos);
                    printf("SHA1...\n");
                    break;
                case 28:
                    g_sha_mode = (SHA_MODE_SHA224 << CRPT_HMAC_CTL_OPMODE_Pos);
                    printf("SHA224...\n");
                    break;
                case 32:
                    printf("SHA256...\n");
                    g_sha_mode = (SHA_MODE_SHA256 << CRPT_HMAC_CTL_OPMODE_Pos);
                    break;
                case 48:
                    printf("SHA384...\n");
                    g_sha_mode = (SHA_MODE_SHA384 << CRPT_HMAC_CTL_OPMODE_Pos);
                    break;
                case 64:
                    printf("SHA512...\n");
                    g_sha_mode = (SHA_MODE_SHA512 << CRPT_HMAC_CTL_OPMODE_Pos);
                    break;
            }
        }
