...
int32_t main(void)
{
    ...
    /*
        Set AES to Nonsecure and check if it can access secure data by DMA
    */

    /* Set CRYPTO to Nonsecure */
    ...
    /* Clear output buffer */
    memset(g_au8Buf, 0, plen_aligned);
    ...
    /* Due to CRYPTO is non-secure. The output buffer should be all zero .*/
    for(i = 0; i < plen_aligned; i++)
    {
        if(g_au8Buf[i] != 0)
        {
            printf("Error! Secure RAM be written by nonsecure CRYPTO.");
            while(1){}
        }
    }
    ...
    if(i32Err)
    {
        /* DMA read secure should fail. So if err it means test PASS */
        printf("TEST PASSED\n");
    }
    else
    {
